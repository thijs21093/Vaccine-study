---
title: "Report COVID study FR V102"
...
```{r}
load("C:/Users/Thijs/surfdrive/COVID vaccine/git/FR/.RData")
...
**Age**
```{r}
age.population <- read.csv("AGE_FR-08022021.csv")
agecut.5 <- c(-Inf, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80, Inf)


age.population <- age.population %>% mutate(agecat = case_when(age < 25 ~ "younger than 25",
          age >= 25 & age < 30 ~ "25-29",
          age >= 30 & age < 35 ~ "30-34",
          age >= 35 & age < 40 ~ "35-39",
          age >= 40 & age < 45 ~ "40-44",
...
age.population.5 <- age.population %>% group_by(agecat) %>% tally(count) %>% mutate(percentage = n / sum(n))

age.total <- merge(age.population.5, age.sample, by.x = "agecat", by.y = ".")
...
**Education**
```{r}
education.population <- read.csv("EDUCATION_FR-08022021.csv")

education.sample.eurostat <-  df.def %>% drop_na(education.recoded)  %>% filter(age <75)  %>%  count(education.recoded) %>% 
  mutate(percentage = n / sum(n))

education.total.eurostat <- merge(education.population, education.sample.eurostat, by.x = "ï..education", by.y = "education.recoded", all = TRUE)

education.total.eurostat <- education.total.eurostat %>% 
  rename(education.category = ï..education,
  count.population = count,
  count.sample = n,
  percentage.population = percentage.x,
  percentage.sample = percentage.y)

my.labels.eurostat <- c('1. Less than primary,\nprimary and\nlower secondary\neducation',
               '2. Upper secondary\nand post-secondary\nnon-tertiary education',
         '3. Tertiary education')

education.total.eurostat %>% select(education.category, percentage.population, percentage.sample) %>% reshape2::melt(id.vars = 'education.category', variable.name = 'series')  %>% ggplot(aes(education.category, value, colour = series, fill = series)) + geom_bar (stat="identity", width=0.9, position = position_dodge(width=0.9)) + 
  theme_blank() + theme(legend.position="bottom", axis.title  = element_blank()) + scale_y_continuous(labels=scales::percent) + scale_x_discrete(labels= my.labels.eurostat)
```
...
**Province**
```{r echo=FALSE}
provinces.population <- read.csv("PROVINCES_FR-08022021.csv",  encoding = "UTF-8")
provinces.sample <-  df.def %>% drop_na(province)  %>%  count(province) %>% 
  mutate(percentage = n / sum(n))

provinces.total <- merge(provinces.sample, provinces.population, by.y = "X.U.FEFF.province", by.x = "province")
...
provinces.total %>% select(province, percentage.population, percentage.sample) %>% reshape2::melt(id.vars = 'province', variable.name = 'series')  %>% ggplot(aes(province, value, colour = series, fill = series)) + geom_bar (stat = "identity", width = 0.5, position = position_dodge(width = 0.9)) + 
  theme_blank() + theme(legend.position="bottom", axis.title  = element_blank(), axis.text.y = element_text(size = 7.5), aspect.ratio = 1.25/1) + coord_flip() + scale_y_continuous(labels=scales::percent)
```
...
### Total
```{r warning=TRUE}
table.prepost <- df.def  %>% 
  select(experimental.group,
         credibility.EMA,
         credibility.post,
         device) %>%
  gather("key","value",-experimental.group, -device) %>% 
  mutate(key = recode(key,"'credibility.EMA'='0.Pre';'credibility.post'='1.Post'"))
  
ggarrange(table.prepost %>% ggplot(aes(x=key,y=value,color=experimental.group))+
  ...
  table.prepost %>% ggplot( aes(x=key,y=value,color=experimental.group))+
      stat_summary(fun.data = mean_cl_normal, geom = "errorbar", alpha=0.5,width=0.1,size=1,position=position_dodge(0.5)) + 
  stat_summary(fun = mean, geom = "point", alpha=0.5,size=2,position=position_dodge(0.5))+
  stat_summary(fun = mean, geom = "line", alpha=0.5,size=1,position=position_dodge(0.5), aes(group = experimental.group)) + facet_grid(vars(device)) +
  theme_tufte(), common.legend = TRUE, legend	= "bottom", ncol = 2)
```

<br >

This code returns a warning. However, the resulting dataframe does not have NAs:
```{r}
colSums(is.na(table.prepost))
```
...
## Distribution of credibility EMA (**pre**)
```{r}
df.def %>%
  ggplot(aes(x = credibility.EMA, fill = experimental.group)) +
  geom_bar(width = 0.75,alpha = 0.8, position = position_dodge()) +
  scale_x_continuous(name="credibility EMA (pre)", breaks = 1:7) + scale_fill_manual(name = "Independence condition") +  scale_fill_viridis_d(name = "Independence condition") + 
  theme_tufte()
```
...
```{r}
df.def %>%
  ggplot(aes(x = credibility.post, fill = experimental.group)) +
  geom_bar(width = 0.75,alpha = 0.8, position = position_dodge()) +
  scale_x_continuous(name="credibility EMA (post)", breaks = 1:7) + scale_fill_manual(name = "Independence condition") +  scale_fill_viridis_d(name = "Independence condition") + 
  theme_tufte()
```
...
reg.perceived.independence2 <- lm(perceived.independence.reversed ~ trust.politicians + trust.scientist + trust.EC + trust.health.authorities + interpersonal.trust + benefits.vaccines + credibility.EMA + familiarity.EMA + familiarity.advice +  political.ideology + consequences.health + consequences.economic +  EU.integration + private.providers + knowledge +  experimental.group + female + poly(age, 2) + healthcare + income,
                    data = df.def,
                    na.action = na.omit)
...
### Perceived independence
```{r}
reg.perceived.independence.30 <- lm(perceived.independence.reversed ~ trust.politicians + trust.scientist + trust.EC + trust.health.authorities + interpersonal.trust + benefits.vaccines + credibility.EMA + familiarity.EMA + familiarity.advice +  political.ideology + consequences.health + consequences.economic +  EU.integration + private.providers + knowledge +  experimental.group + female + poly(age, 2) + healthcare + income,
                    data = df.30,
                    na.action = na.omit)

reg.perceived.independence.check <- lm(perceived.independence.reversed ~  trust.politicians + trust.scientist + trust.EC + trust.health.authorities + interpersonal.trust + benefits.vaccines + credibility.EMA + familiarity.EMA + familiarity.advice +  political.ideology + consequences.health + consequences.economic +  EU.integration + private.providers + knowledge +  experimental.group + female + poly(age, 2) + healthcare + income,
                    data = df.check,
                    na.action = na.omit)
