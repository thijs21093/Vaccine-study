---
title: "Report COVID study NL  212"
...
```{r}
load("C:/Users/Thijs/surfdrive/COVID vaccine/git/NL/.RData")
...
```{r}
give.n <- function(x){
  return(c(y = mean(x)*1.13, label = length(x))) 
}

ggline(df.def, x = "manipulation.submit.quartile", y = "perceived.independence.reversed", color = "experimental.group", ylab = "Perceived independence", xlab = "Page submission - manipulation (quartiles)",  add = "mean_ci", ylim = c(3, 4.25), yticks.by = 0.25) +
...
```{r}
ggline(df.def, x = "manipulation.submit.30", y = "perceived.independence.reversed", color = "experimental.group", ylab = "Perceived independence", xlab = "Page submission - manipulation",  add = c("mean_ci"), ylim = c(3, 4.25), yticks.by = 0.25) +
  stat_summary(fun.data = give.n, geom = "text", fun = median, size = 4, col = "black")
```
...
```{r}
age.population.5 <- read.csv("C:/Users/Thijs/surfdrive/COVID vaccine/git/NL/AGE_POPULATION_SEGMENTS_NL-22012021.csv")
...
Compare with true population:
 ...
 ![Source: CBS.](C:/Users/Thijs/surfdrive/COVID vaccine/git/NL/education.jpeg)
...
Compare with true population:
```{r}
provinces <- read.csv("C:/Users/Thijs/surfdrive/COVID vaccine/git/NL/PROVINCES_NL-22012021.csv")
...
In both models, there is an issue with the 'expert.based' latent variable, which has negative variance. Further investigation is needed to determine the cause of this problem. Theoretically, it would make sense to add a second order as we expect the three dimensions to be related. That is, we see these dimensions as part of a bigger latent variable ("policy credibility").
