...
t1 <- df.total %>%
  group_by(country) %>% 
  tally() %>% 
  mutate(percentage = (n/nrow(df.total)*100) %>% round(1)) %>%
  adorn_totals("row")
...
```{r}
t2 <- df.total %>% filter(IMC != 1) %>%  
  group_by(country) %>% 
  tally() %>% 
  mutate(percentage = ((n/nrow(df.total %>% filter(IMC != 1))*100) %>% round(1))) %>%
  adorn_totals("row")

t3 <- merge(x = t1, y = t2, by = "country", all.x = TRUE) %>%  mutate(n.final = (n.x-n.y) %>% round(2),
        percentage.final = (n.final/nrow(pooled)*100) %>% round(1),
...
```{r}
t5 <- pooled %>% 
sjmisc::descr(perceived.independence.reversed, show = c("n", "NA.prc", "mean", "sd", "md", "iqr", "trimmed", "range", "skew")) %>% mutate(var = case_when(
  var == "perceived.independence.reversed" ~ "Perceived indepedence"))
...
t6 <- pooled %>%
  ...
  geom_bar(width = 0.8, position = position_dodge()) + labs(y = "", title = "Total") +
...
t7 <- pooled %>%
  ggplot(aes(x = perceived.independence.reversed, fill = experimental.group)) +
  geom_bar(width = 0.9, position = position_dodge()) + scale_y_continuous(name="", breaks = c(0, 100, 200)) +
  scale_x_continuous(name="", breaks = 1:7) + scale_fill_manual(name = "Independence condition") +  scale_fill_viridis_d(name = "Independence condition") + facet_wrap(~country) + theme(strip.text = element_text(hjust = 0, face = "bold"),
    aspect.ratio = 1,
    strip.background = element_blank(),
    strip.placement = "outside")

ggarrange(t6, t7, common.legend = TRUE, legend = "bottom")
```
...
### Total
```{r warning=TRUE}
table.prepost <- pooled  %>% 
  select(experimental.group,
         credibility.EMA.pre,
         credibility.EMA.post,
         device) %>% drop_na(credibility.EMA.post) %>%
