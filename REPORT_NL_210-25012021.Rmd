---
title: "Report COVID study NL  210"
...
```{r}
load("C:/Users/Thijs/surfdrive/COVID vaccine/R data/NL/.RData")

source("C:/Users/Thijs/surfdrive/COVID vaccine/R data/Vaccine study/DATA_MANIPULATION_NL-21012021.R")
...
Number of participants that failed the manipulation check:
```{r}
df.total %>% nrow() - df.total %>% filter(manipulation.check=="1") %>% nrow()
```
...
table.duration.grouped <- df.def %>% group_by(manipulation.check) %>%
  summarize(mean = mean(duration),
            sd = sd(duration),
            min = min(duration),
            q1 = quantile(duration, probs = 0.25),
...
table.intro.grouped <- df.def %>% group_by(manipulation.check) %>%  summarize(mean = mean(intro.submit),
            sd = sd(intro.submit),
...
table.intro.submit <- df.def %>% 
  summarize(mean = mean(intro.submit),
            sd = sd(intro.submit),
            min = min(intro.submit),
            q1 = quantile(intro.submit, probs = 0.25),
            median = median(intro.submit),
...
table.intro.submit.grouped <- df.def %>% group_by(manipulation.check) %>% 
  summarize(mean = mean(intro.submit),
            sd = sd(intro.submit),
            min = min(intro.submit),
            q1 = quantile(intro.submit, probs = 0.25),
...
table.manipulation.submit <- df.def %>%  
  summarize(mean = mean(manipulation.submit),
...
table.manipulation.submit.grouped <- df.def %>%  group_by(manipulation.check) %>%
  summarize(mean = mean(manipulation.submit),
            sd = sd(manipulation.submit),
            min = min(manipulation.submit),
            q1 = quantile(manipulation.submit, probs = 0.25),
...
table.decision.submit <- df.def %>% 
  summarize(mean = mean(decision.submit),
...
table.decision.submit.grouped <- df.def %>% group_by(manipulation.check) %>%
  summarize(mean = mean(decision.submit),
            sd = sd(decision.submit),
            min = min(decision.submit),
            q1 = quantile(decision.submit, probs = 0.25),
...
plot_frq((duration),
        ...
        type = "hist",
        geom.size = 0.25) +
  theme(axis.text = element_text(face = "bold",  size= 9)) + labs(title = "Total survey duration") +  scale_x_continuous(name="Time in minutes", breaks = seq(0,30, 2)) + ylim(0, 30)+ annotate(geom = "table", x = Inf, y = Inf, label = list(table.duration))
```
...
plot_frq((intro.submit),
  ...
  scale_x_continuous(name="Time in seconds",breaks = seq(0,100,5)) + ylim(0, 75) + annotate(geom = "table", x = Inf, y = Inf, label = list(table.intro.submit))
```
...
plot_frq((manipulation.submit),
        ...
        geom.size = 1) +
  theme(axis.text = element_text(face = "bold",  size= 9)) +
  scale_x_continuous(name="Time in seconds",breaks = seq(0,100,5)) + ylim(0, 75) + annotate(geom = "table", x = Inf, y = Inf, label = list(table.manipulation.submit))
```
...
ggline(df.def, x = "manipulation.submit.quartile", y = "perceived.independence.reversed", color = "experimental.group", ylab = "Perceived independence", xlab = "Page submission - manipulation (quartiles)",  add = "mean_se", ylim = c(3, 4.25), yticks.by = 0.25) +
...
```{r}
ggline(df.def, x = "manipulation.submit.30", y = "perceived.independence.reversed", color = "experimental.group", ylab = "Perceived independence", xlab = "",  add = c("mean_se"), ylim = c(3, 4.25), yticks.by = 0.25) +
  stat_summary(fun.data = give.n, geom = "text", fun = median, size = 4, col = "black")
```
...
plot_frq((decision.submit),
        ...
        geom.size = 1) + 
  theme(axis.text = element_text(face = "bold",  size= 9)) +
  scale_x_continuous(name= "Time in seconds", breaks = seq(0,100,5)) + ylim(0, 75) + annotate(geom = "table", x = Inf, y = Inf, label = list(table.decision.submit))
```
...
Sample:
```{r}
age.population.1 <- read.csv("AGE_POPULATION_NL-22012021.csv")
...
Comparing age in sample and true population:
```{r}
agecut <- c(-Inf, 25, 30, 35, 40, 46, 50, 55, 60, 65, 70, 75, 80, Inf)
age.sample <- df.def %>% transmute(agecat = cut(age, agecut, c("younger than 25", "25-29", "30-34", "35-39", "40-44", "45-49", "50-54", "55-59", "60-64", "65-69", "70-74", "75-79", "80 or older"), right=FALSE))  %>% table() %>% as_data_frame()

age.sample <- age.sample %>% 
  mutate(percentage = n / sum(n))
         
age.population.5 <- read.csv("AGE_POPULATION_SEGMENTS_NL-22012021.csv")
...
```{r}
df.def %>%
  filter(education.recoded!="0") %>%
  arrange(education.recoded)  %>%
  plot_frq(education.recoded,
        ylim = c(0,nrow(df.def)/1.5),
...
Compare with true population:

 ![Source: CBS.](Embed/education.jpeg)
...
df.def %>% 
  plot_frq(province,
        ylim = c(0,nrow(df.def)/4),
        title = "Province: sample") + 
  theme_blank()+
...
Compare with true population:
```{r}
provinces <- read.csv("PROVINCES_NL-22012021.csv")
provinces %>% 
  rename(province = ï..province)  %>% 
  arrange(province) %>%
ggplot(aes(x = province, y = percentage)) + geom_bar(stat="identity") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title  = element_blank()) + labs(title = "Province: true population") + scale_y_continuous(labels=scales::percent)
```
 source: CBS.
...
```{r}
df.def %>% 
  select(trust.EC:trust.politicians, trust.health.authorities, benefits.vaccines, credibility.index, credibility.EMA, familiarity.EMA, familiarity.advice, consequences.economic, consequences.health, age, perceived.independence.reversed, political.ideology, EU.integration, knowledge, interpersonal.trust, treatment) %>% 
...
```{r}
reg.perceived.independence1 <- lm(perceived.independence.reversed ~  trust.politicians + trust.scientist + trust.health.authorities + benefits.vaccines + credibility.EMA + familiarity.EMA + political.ideology + EU.integration + knowledge + interpersonal.trust + experimental.group + female + poly(age, 2) + healthcare + income + education.recoded,
                    data = df.def,
                    na.action = na.omit)

reg.perceived.independence2 <- lm(perceived.independence.reversed ~ credibility.index + trust.politicians + trust.scientist + trust.health.authorities + benefits.vaccines + credibility.EMA + familiarity.EMA + political.ideology + EU.integration + knowledge + interpersonal.trust + experimental.group +  female + poly(age, 2) + healthcare + income + education.recoded,
                    data = df.def,
                    na.action = na.omit)
...
```{r}
reg.intent.vaccine1 <- lm(intent.vaccine ~ trust.politicians + trust.scientist + trust.health.authorities + benefits.vaccines + credibility.EMA + familiarity.EMA + political.ideology +  interpersonal.trust + EU.integration + knowledge + experimental.group + female + poly(age, 2) + healthcare + income + education.recoded,
                    data = df.def,
                    na.action = na.omit)

reg.intent.vaccine2 <- lm(intent.vaccine ~ credibility.index + trust.politicians + trust.scientist + trust.health.authorities + benefits.vaccines + credibility.EMA + familiarity.EMA + political.ideology + interpersonal.trust + EU.integration + knowledge + experimental.group +  female + poly(age, 2) + healthcare + income + education.recoded,
                    data = df.def,
                    na.action = na.omit)

reg.intent.vaccine3 <- lm(intent.vaccine ~ perceived.independence.reversed + trust.politicians + trust.scientist + trust.health.authorities + benefits.vaccines + credibility.EMA + familiarity.EMA + political.ideology + interpersonal.trust + EU.integration + knowledge + experimental.group +  female + poly(age, 2) + healthcare + income + education.recoded,
                    data = df.def,
                    na.action = na.omit)

reg.intent.vaccine4 <- lm(intent.vaccine ~ credibility.index + perceived.independence.reversed + trust.politicians + trust.scientist + trust.health.authorities + benefits.vaccines + credibility.EMA + familiarity.EMA + political.ideology + interpersonal.trust + EU.integration + knowledge + experimental.group +  female + poly(age, 2) + healthcare + income + education.recoded,
                    data = df.def,
                    na.action = na.omit)
...
```{r}
reg.credibility1 <- lm(credibility.index  ~ trust.politicians + trust.scientist + trust.health.authorities + benefits.vaccines + credibility.EMA + familiarity.EMA + political.ideology + interpersonal.trust + EU.integration + knowledge + experimental.group + female + poly(age, 2) + healthcare + income + education.recoded,
                    data = df.def,
                    na.action = na.omit)

reg.credibility2 <- lm(credibility.index  ~ perceived.independence.reversed + trust.politicians + trust.scientist + trust.health.authorities + benefits.vaccines + credibility.EMA + familiarity.EMA + political.ideology + interpersonal.trust + EU.integration + knowledge + experimental.group + female + poly(age, 2) + healthcare + income + education.recoded,
                    data = df.def,
                    na.action = na.omit)

tab_model(reg.credibility1, reg.credibility2, show.ci = FALSE)
```
