...
```{r}
load("C:/Users/Thijs/surfdrive/COVID vaccine/R data/git/SE/.RData")
...
ggline(df.def, x = "manipulation.submit.quartile", y = "perceived.independence.reversed", color = "experimental.group", ylab = "Perceived independence", xlab = "Page submission - manipulation (quartiles)",  add = "mean_se", ylim = c(2, 4), yticks.by = 0.5) +
...
education.sample <-  df.total %>% drop_na(education.recoded)  %>%  count(education.recoded) %>% 
  mutate(percentage = n / sum(n))
...
provinces.sample <-  df.total %>% drop_na(province)  %>%  count(province) %>% 
  mutate(percentage = n / sum(n))
